...
def get_hierarchical_mcvc(G):
    ...
    while True:
        ...
        temp = nx.subgraph(G, mcvc)
        if temp.size() == len(mcvc) - 1:
            mcvc = sort_chain(temp)
...
def get_mcvc_mapping(G, mcvc):
    ...
    mapping = {}
    mapping = dict((el,dict()) for el in range(len(mcvc)))
    ...
    for vertex in G.nodes():
        if vertex in mcvc:
            ...
            if degree == 1:
                ...
                position = mcvc.index(key)

                mapping[position]['exclusive'] += 1

                #print("Exclusive neighbor " + str(vertex) + " for " + str(key))

            # This node is a common neighbor for multiple mcvc nodes
            elif degree > 1:
                mcvc_vertices = [mcvc.index(edge[1]) for edge in G.edges(vertex)]
